...
public class AuthService {
    ...
    public AuthService(UserRepository userRepository, SessionRepository sessionRepository, BCryptPasswordEncoder bCryptPasswordEncoder) {
        ...
        this.bCryptPasswordEncoder = bCryptPasswordEncoder;//Injected the dependency with out "new BCryptPasswordEncoder()" we have use @Bean to know spring to know its need to initialise
    }
    ...
    public ResponseEntity<UserDto> login(String email, String password) {
        ...
        if (!bCryptPasswordEncoder.matches(password, user.getPassword())) {//NOTE 5: we cant use !user.getPassword().equals(password) because it is not possible to decrypt the given string
        // so we use the match function so it will check if the algorithm can generate this password or not
