...
public class SecurityConfig {
  ...
  public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http)
      throws Exception { // NOTE : to allow the api we or restrict we can handle here
    ...
    http
        ...
        // NOTE 31: resource server is like gmail
        // build the http
  ...
  public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
    http.authorizeHttpRequests((authorize) -> authorize.anyRequest().authenticated())
        ...
        // NOTE 32 UP : if login is not done by the user from where it want to
        // login then redirect him to login page of fb, gmail, linkedIn or anyother
        // third party login
  ...
  */
  ...
  // NOTE 49 UP : How-to: Add authorities as custom claims in JWT access tokens
  /*
   @Bean
  ...
  */
  ...
  // NOTE 50 UP :
  // customise the token and add role in the payload
  @Bean
  public OAuth2TokenCustomizer<JwtEncodingContext> jwtTokenCustomizer() {
    return (context) -> {
      if (OAuth2TokenType.ACCESS_TOKEN.equals(context.getTokenType())) {
